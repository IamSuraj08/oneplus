

export const storeProducts = [
  {
    id: 1,
    title: "Oneplus - 1",
    img: "img/oneplus-1.jpg",
    price: 10,
    company: "Oneplus",
    info:
        "Screen size (inches)\t5.50\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x1920 pixels\n" +
        "Aspect ratio\t16:9\n" +
        "Pixels per inch (PPI)\t401\n" +
        "Hardware\n" +
        "Processor\t2.5GHz quad-core\n" +
        "Processor make\tQualcomm Snapdragon 801\n" +
        "RAM\t3GB\n" +
        "Internal storage\t16GB\n" +
        "Expandable storage\tNo",
    inCart: false,
    count: 0,
    total: 0
  },
  {
    id: 2,
    title: "Oneplus - X",
    img: "img/oneplus-x.jpg",
    price: 16,
    company: "Oneplus",
    info:
      "Screen size (inches)\t5.00\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x1920 pixels\n" +
        "Pixels per inch (PPI)\t441\n" +
        "Hardware\n" +
        "Processor\t2.3GHz quad-core\n" +
        "Processor make\tQualcomm Snapdragon 801\n" +
        "RAM\t3GB\n" +
        "Internal storage\t16GB\n" +
        "Expandable storage\tYes\n" +
        "Expandable storage type\tmicroSD\n" +
        "Expandable storage up to (GB)\t128",
    inCart: false,
    count: 0,
    total: 0
  },
  {
    id: 3,
    title: "Oneplus - 2",
    img: "img/oneplus-2.jpg",
    price: 8,
    company: "Oneplus",
    info:
      "Screen size (inches)\t5.50\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x1920 pixels\n" +
        "Hardware\n" +
        "Processor\t1.8GHz octa-core\n" +
        "Processor make\tQualcomm Snapdragon 810\n" +
        "RAM\t4GB\n" +
        "Internal storage\t64GB\n" +
        "Expandable storage\tNo",
    inCart: false,
    count: 0,
    total: 0
  },
  {
    id: 4,
    title: "Oneplus - 3",
    img: "img/oneplus-3.jpg",
    price: 18,
    company: "Oneplus",
    info:
      "Screen size (inches)\t5.50\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x1920 pixels\n" +
        "Pixels per inch (PPI)\t401\n" +
        "Hardware\n" +
        "Processor\t1.6GHz quad-core\n" +
        "Processor make\tQualcomm Snapdragon 820\n" +
        "RAM\t6GB\n" +
        "Internal storage\t64GB\n" +
        "Expandable storage\tNo\n" +
        "Camera\n" +
        "Rear camera\t16-megapixel\n" +
        "Rear flash\tLED\n" +
        "Front camera\t8-megapixel",
    inCart: false,
    count: 0,
    total: 0
  },
  {
    id: 5,
    title: "Oneplus - 3T",
    img: "img/oneplus-3t.jpg",
    price: 24,
    company: "Oneplus",
    info:
      "Screen size (inches)\t5.50\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x1920 pixels\n" +
        "Pixels per inch (PPI)\t401\n" +
        "Hardware\n" +
        "Processor\t1.6GHz quad-core\n" +
        "Processor make\tQualcomm Snapdragon 821\n" +
        "RAM\t6GB\n" +
        "Internal storage\t128GB\n" +
        "Expandable storage\tNo\n" +
        "Camera\n" +
        "Rear camera\t16-megapixel\n" +
        "Rear flash\tLED\n" +
        "Front camera\t16-megapixel",
    inCart: false,
    count: 0,
    total: 0
  },
  {
    id: 6,
    title: "Oneplus - 5",
    img: "img/oneplus-5.jpg",
    price: 17,
    company: "Oneplus",
    info:
      "Screen size (inches)\t5.50\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x1920 pixels\n" +
        "Hardware\n" +
        "Processor\t1.9GHz octa-core\n" +
        "Processor make\tQualcomm Snapdragon 835\n" +
        "RAM\t8GB\n" +
        "Internal storage\t128GB\n" +
        "Expandable storage\tNo\n" +
        "Camera\n" +
        "Rear camera\t16-megapixel\n" +
        "Rear flash\tDual LED\n" +
        "Front camera\t16-megapixel",
    inCart: false,
    count: 0,
    total: 0
  },
  {
    id: 7,
    title: "Oneplus - 5T",
    img: "img/oneplus-5t.jpg",
    price: 17,
    company: "Oneplus",
    info:
      "Screen size (inches)\t6.01\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x2160 pixels\n" +
        "Protection type\tGorilla Glass\n" +
        "Aspect ratio\t18:9\n" +
        "Pixels per inch (PPI)\t401\n" +
        "Hardware\n" +
        "Processor\tocta-core (4x2.45GHz + 4x1.9GHz)\n" +
        "Processor make\tQualcomm Snapdragon 835\n" +
        "RAM\t6GB\n" +
        "Internal storage\t64GB\n" +
        "Expandable storage\tNo\n" +
        "Camera\n" +
        "Rear camera\t16-megapixel (f/1.7, 1.12-micron) + 20-megapixel (f/1.7, 1.0-micron)\n" +
        "Rear autofocus\tPhase detection autofocus\n" +
        "Rear flash\tDual LED\n" +
        "Front camera\t16-megapixel (f/2.0, 1.0-micron)",
    inCart: false,
    count: 0,
    total: 0
  },
  {
    id: 8,
    title: "Oneplus - 6",
    img: "img/oneplus-6.jpg",
    price: 30,
    company: "Oneplus",
    info:
      "Screen size (inches)\t6.28\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x2280 pixels\n" +
        "Protection type\tGorilla Glass\n" +
        "Aspect ratio\t19:9\n" +
        "Pixels per inch (PPI)\t402\n" +
        "Hardware\n" +
        "Processor\t2.8GHz octa-core (4x2.8GHz + 4x1.7GHz)\n" +
        "Processor make\tQualcomm Snapdragon 845\n" +
        "RAM\t6GB\n" +
        "Internal storage\t64GB\n" +
        "Expandable storage\tNo\n" +
        "Camera\n" +
        "Rear camera\t16-megapixel (f/1.7, 1.22-micron) + 20-megapixel (f/1.7, 1.0-micron)\n" +
        "Rear autofocus\tPhase detection autofocus\n" +
        "Rear flash\tDual LED\n" +
        "Front camera\t16-megapixel (f/2.0, 1.0-micron)",
    inCart: false,
    count: 0,
    total: 0
  },
  {
    id: 9,
    title: "Oneplus - 6T",
    img: "img/oneplus-6t.jpg",
    price: 2,
    company: "Oneplus",
    info:
      "Screen size (inches)\t6.41\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x2340 pixels\n" +
        "Protection type\tGorilla Glass\n" +
        "Aspect ratio\t19.5:9\n" +
        "Hardware\n" +
        "Processor\tocta-core (4x2.8GHz)\n" +
        "Processor make\tQualcomm Snapdragon 845\n" +
        "RAM\t8GB\n" +
        "Internal storage\t128GB\n" +
        "Expandable storage\tNo\n" +
        "Camera\n" +
        "Rear camera\t16-megapixel (f/1.7, 1.22-micron) + 20-megapixel (f/1.7, 1-micron)\n" +
        "Rear autofocus\tPhase detection autofocus\n" +
        "Rear flash\tDual LED\n" +
        "Front camera\t16-megapixel (f/2.0, 1-micron)\n" +
        "Front flash\tNo",
    inCart: false,
    count: 0,
    total: 0
  },
  {
    id: 10,
    title: "Oneplus -6T McLaren Edition",
    img: "img/oneplus-6t-mclaren-edition.jpg",
    price: 2,
    company: "Oneplus",
    info:
      "Screen size (inches)\t6.41\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x2340 pixels\n" +
        "Protection type\tGorilla Glass\n" +
        "Aspect ratio\t19.5:9\n" +
        "Hardware\n" +
        "Processor\tocta-core (4x2.8GHz)\n" +
        "Processor make\tQualcomm Snapdragon 845\n" +
        "RAM\t10GB\n" +
        "Internal storage\t256GB\n" +
        "Expandable storage\tNo\n" +
        "Camera\n" +
        "Rear camera\t16-megapixel (f/1.7, 1.22-micron) + 20-megapixel (f/1.7, 1-micron)\n" +
        "Rear autofocus\tPhase detection autofocus\n" +
        "Rear flash\tDual LED\n" +
        "Front camera\t16-megapixel (f/2.0, 1-micron)\n" +
        "Front flash\tNo",
    inCart: false,
    count: 0,
    total: 0
  }
  ,{
    id: 11,
    title: "Oneplus - 7",
    img: "img/oneplus-7.jpg",
    price: 2,
    company: "Oneplus",
    info:
      "Screen size (inches)\t6.41\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x2340 pixels\n" +
        "Protection type\tGorilla Glass\n" +
        "Aspect ratio\t19.5:9\n" +
        "Pixels per inch (PPI)\t402\n" +
        "Hardware\n" +
        "Processor\tocta-core\n" +
        "Processor make\tQualcomm Snapdragon 855\n" +
        "RAM\t8GB\n" +
        "Internal storage\t256GB\n" +
        "Expandable storage\tNo\n" +
        "Camera\n" +
        "Rear camera\t48-megapixel (f/1.7, 1.6-micron) + 5-megapixel (f/2.4, 1.12-micron)\n" +
        "Rear autofocus\tPhase detection autofocus\n" +
        "Rear flash\tYes\n" +
        "Front camera\t16-megapixel (f/2.0, 1.0-micron)\n" +
        "Front autofocus\tNo",
    inCart: false,
    count: 0,
    total: 0
  }
  ,{
    id: 12,
    title: "Oneplus - 7T",
    img: "img/oneplus-7t.jpg",
    price: 2,
    company: "Oneplus",
    info:
      "Screen size (inches)\t6.55\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1080x2400 pixels\n" +
        "Protection type\tGorilla Glass\n" +
        "Pixels per inch (PPI)\t402\n" +
        "Hardware\n" +
        "Processor\tocta-core\n" +
        "Processor make\tQualcomm Snapdragon 855+\n" +
        "RAM\t8GB\n" +
        "Internal storage\t128GB\n" +
        "Expandable storage\tNo\n" +
        "Dedicated microSD slot\tNo\n" +
        "Camera\n" +
        "Rear camera\t48-megapixel (f/1.6, 0.8-micron) + 16-megapixel (f/2.2) + 12-megapixel (f/2.2)\n" +
        "Rear autofocus\tPhase detection autofocus\n" +
        "Rear flash\tDual LED\n" +
        "Front camera\t16-megapixel (f/2.0, 1.0-micron)\n" +
        "Front autofocus\tNo\n" +
        "Front flash\tNo",
    inCart: false,
    count: 0,
    total: 0
  },
  {
    id: 13,
    title: "Oneplus - 7T Pro",
    img: "img/oneplus-7t-pro.jpg",
    price: 2,
    company: "Oneplus",
    info:
      "Screen size (inches)\t6.67\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1440x3120 pixels\n" +
        "Protection type\tGorilla Glass\n" +
        "Aspect ratio\t19.5:9\n" +
        "Pixels per inch (PPI)\t516\n" +
        "Hardware\n" +
        "Processor\tocta-core\n" +
        "Processor make\tQualcomm Snapdragon 855+\n" +
        "RAM\t8GB\n" +
        "Internal storage\t256GB\n" +
        "Expandable storage\tNo\n" +
        "Camera\n" +
        "Rear camera\t48-megapixel (f/1.6, 0.8-micron) + 16-megapixel (f/2.2) + 8-megapixel (f/2.4, 1.0-micron)\n" +
        "Rear autofocus\tYes\n" +
        "Rear flash\tYes\n" +
        "Front camera\t16-megapixel (f/2.0, 1.0-micron)",
    inCart: false,
    count: 0,
    total: 0
  },
  {
    id: 14,
    title: "Oneplus - 7T Pro McLaren Edition",
    img: "img/oneplus-7t-pro-mclaren-edition.jpg",
    price: 2,
    company: "Oneplus",
    info:
      "Screen size (inches)\t6.67\n" +
        "Touchscreen\tYes\n" +
        "Resolution\t1440x3120 pixels\n" +
        "Protection type\tGorilla Glass\n" +
        "Aspect ratio\t19.5:9\n" +
        "Pixels per inch (PPI)\t516\n" +
        "Hardware\n" +
        "Processor\tocta-core\n" +
        "Processor make\tQualcomm Snapdragon 855+\n" +
        "RAM\t12GB\n" +
        "Internal storage\t256GB\n" +
        "Expandable storage\tNo\n" +
        "Camera\n" +
        "Rear camera\t48-megapixel (f/1.6, 0.8-micron) + 16-megapixel (f/2.2) + 8-megapixel (f/2.4, 1.0-micron)\n" +
        "Rear autofocus\tYes\n" +
        "Rear flash\tYes\n" +
        "Front camera\t16-megapixel (f/2.0, 1.0-micron)",
    inCart: false,
    count: 0,
    total: 0
  }
];

export const detailProduct = {
  id: 1,
  title: "Oneplus - 1",
  img: "img/oneplus-1.jpg",
  price: 10,
  company: "Oneplus",
  info:
      "Screen size (inches)\t5.50\n" +
      "Touchscreen\tYes\n" +
      "Resolution\t1080x1920 pixels\n" +
      "Aspect ratio\t16:9\n" +
      "Pixels per inch (PPI)\t401\n" +
      "Hardware\n" +
      "Processor\t2.5GHz quad-core\n" +
      "Processor make\tQualcomm Snapdragon 801\n" +
      "RAM\t3GB\n" +
      "Internal storage\t16GB\n" +
      "Expandable storage\tNo",
  inCart: false,
  count: 0,
  total: 0
};


